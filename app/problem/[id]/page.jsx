...
const getCodeSnippet = (codeSnippets, language) => {
...
const ProblemIdPage = ({ params }) => {
    ...
    const fetchProblem = async () => {
      try {
        ...
        if (problemData.success) {
          ...
          setLoadError(problemData.error || "Failed to load problem");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (problem) {
      setCode(getCodeSnippet(problem.codeSnippets, selectedLanguage));
    }
  ...
  const runTests = async ({ isSubmit = false } = {}) => {
    try {
      ...
      if (res.allPassed) {
        toast.success(
          isSubmit ? "Accepted! All test cases passed." : res.message
        );
        ...
        toast.error(res.message || "Some test cases failed");
      }
    } catch (error) {
  ...
  const handleRun = () => runTests({ isSubmit: false });
  const handleSubmit = () => runTests({ isSubmit: true });
            ...
            <Card>
              ...
              <CardContent>
                <div className="border rounded-lg overflow-hidden">
                  <Editor
                    height="400px"
                    language={MONACO_LANGUAGE[selectedLanguage] ?? "javascript"}
                    value={code ?? ""}
